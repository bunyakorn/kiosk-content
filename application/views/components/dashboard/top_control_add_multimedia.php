...
function onSubmit() {
    ...
    if (file_video == "") {
        Swal.fire(
                'กรุณาตรวจสอบข้อมูล',
                'ท่านยังไม่ได้เพิ่มไฟล์ วิดีโอมัลติมีเดีย',
                'warning'
            )
            .then((result) => {
                $("#file_video").focus().trigger('click');
            });
    } else if (name_multimedia == "") {
        Swal.fire(
                'กรุณาตรวจสอบข้อมูล',
...
<div class="bg-white p-3 header-secondary row">
    ...
    <div class="col col-auto">
        <a class="btn btn-light mt-4 mt-sm-0" href="<?php echo BASEURL;?>/multimedia">
