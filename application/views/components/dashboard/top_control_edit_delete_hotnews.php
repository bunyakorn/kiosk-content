<script>
    function onUpdate() {
        var name_hotnews = $("#name_hotnews").val();
        var category = $("#category").val();
        var dateail_hotnews = $("#dateail_hotnews").val();
        //-------------------------------------------
        ...
        if (name_hotnews == "") {
            Swal.fire(
                    'กรุณาตรวจสอบข้อมูล',
                    'ท่านยังไม่ได้กรอกหัวข้อการประชาสัมพันธ์ ( Hot News )',
                    'warning'
                )
                .then((result) => {
                    $("#name_hotnews").focus().trigger('click');
                });
        } else if (category == "") {
            Swal.fire(
                    'กรุณาตรวจสอบข้อมูล',
        ...
        } else if (dateail_hotnews == "") {
            Swal.fire(
                    'กรุณาตรวจสอบข้อมูล',
                    'ท่านยังไม่ได้กรอกข้อมูลรายละเอียดที่ต้องการให้แสดงเพิ่มเติม',
                    'warning'
                )
                .then((result) => {
                    $("#dateail_hotnews").focus().trigger('click');
                });
        } else if (date_start == "") {
            Swal.fire(
                    'กรุณาตรวจสอบข้อมูล',
...
<div class="bg-white p-3 header-secondary row">
    ...
    <div class="col col-auto">
        <a class="btn btn-light mt-4 mt-sm-0" href="<?php echo BASEURL;?>/editHotnews">
