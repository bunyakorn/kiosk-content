    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">ตารางแสดงรายการข้อความวิ่ง</h3>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <?php 
                                    ...
                                    switch ($show) {
                                        case (string) 1:
                                            echo '<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-eye" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <td class="text-center">
                                <a class="btn btn-sm btn-indigo" href="javaScript:void(0);"
                                    id="<?php echo $row->hnid;?>" onclick="editHotnews(this.id)">
                                    <i class="fa fa-edit"></i>
                                    แก้ไข
                                </a>
                                <a class="btn btn-sm btn-instagram" href="javaScript:void(0);"
                                    id="<?php echo $row->hnid;?>" onclick="deleteHotnews(this.id)">
...
function editHotnews(editID) {
    var searchValueID = (editID);
    $("#btnUpdate").removeClass("hide");
    $("#form_update").removeClass("hide");
    var urlForm = '<?php echo BASEURL; ?>/editHotnews/formEditHotnews?hhnewID=' + searchValueID;
    if(searchValueID !== ''){
...
function deleteHotnews(delID) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            var urlDel = '<?php echo BASEURL; ?>/editHotnews/deleteHotnews?dataID=' + deleteId;
            $.get(urlDel, function(returnCode){
                if(returnCode === "success"){
                    ...
                    .then((result) => {
                        window.location.href = "<?php echo BASEURL; ?>/editHotnews";
                    });
                }else{
                    Swal.fire(
                        'ลบข้อมูลไม่สำเร็จ',
